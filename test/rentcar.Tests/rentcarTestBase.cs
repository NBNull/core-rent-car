...
namespace rentcar.Tests
{
    public abstract class rentcarTestBase : AbpIntegratedTestBase<rentcarTestModule>
    {
        ...
        protected void UsingDbContext(Action<rentcarDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<rentcarDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<rentcarDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<rentcarDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<rentcarDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<rentcarDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<rentcarDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<rentcarDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<rentcarDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<rentcarDbContext, Task<T>> func)
        {
            T result;
